...
# --- [নতুন] গুগল ড্রাইভ লিংককে এমবেড লিংকে রূপান্তর করার জন্য Helper ফাংশন ---
def convert_gdrive_link_to_embed(url):
    """
    Takes a Google Drive sharing URL and converts it to a direct embed/preview URL.
    ...
    if not url or 'drive.google.com' not in url:
        return url # যদি গুগল ড্রাইভের লিংক না হয়, তাহলে যা আছে তাই ফেরত দেবে
    ...
    return url # যদি আইডি খুঁজে না পাওয়া যায়, তাহলেও মূল লিংক ফেরত দেবে
...
def send_telegram_notification(movie_data, content_id, notification_type='new', series_update_info=None):
    ...
    try:
        ...
        inline_keyboard = {"inline_keyboard": [[{"text": "📺👇 Watch Now 👇📺", "url": movie_url}]]}
...
# --- [সম্পূর্ণ পরিবর্তিত] Detail Page Template with Modal Player ---
detail_html = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<style>
  ...
  .back-link { display: inline-block; margin-bottom: 20px; padding: 8px 15px; background-color: var(--card-bg); color: var(--text-dark); border-radius: 50px; font-size: 0.9rem; transition: all 0.2s ease; }
  .back-link:hover { color: var(--text-light); background-color: #333; }
  .back-link i { margin-right: 8px; }
  
  .hero-section { position: relative; width: 100%; max-width: 900px; margin: 20px auto 80px; aspect-ratio: 16 / 9; background-size: cover; background-position: center; border-radius: 12px; box-shadow: 0 0 25px rgba(0, 255, 255, 0.4); overflow: visible; }
  .hero-poster { position: absolute; left: 30px; bottom: -60px; height: 95%; aspect-ratio: 2 / 3; object-fit: cover; border-radius: 8px; box-shadow: 0 8px 25px rgba(0,0,0,0.6); border: 2px solid rgba(255, 255, 255, 0.1); }
  .content-title-section { text-align: center; padding: 10px 15px 30px; max-width: 900px; margin: 0 auto; }
  .main-title { font-family: 'Oswald', sans-serif; font-size: clamp(1.8rem, 5vw, 2.5rem); font-weight: 700; line-height: 1.4; color: var(--cyan-accent); text-transform: uppercase; }
  ...
  /* --- [নতুন] ভিডিও প্লেয়ার মডাল স্টাইল --- */
  ...
  .video-modal-content { position: relative; width: 95%; max-width: 900px; background-color: #000; }
  .video-player-wrapper { position: relative; padding-top: 56.25%; /* 16:9 Aspect Ratio */ height: 0; }
  .video-player-wrapper iframe { position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: none; }
  .close-modal-btn { position: absolute; top: -40px; right: -5px; font-size: 2.5rem; color: white; background: transparent; border: none; cursor: pointer; z-index: 10000; }

  .ad-container { margin: 20px auto; display: flex; justify-content: center; }
...
<main class="container">
    ...
    <div class="tabs-content">
        ...
        <div class="tab-pane active" id="watch-pane">
                ...
                {% for season_num in all_seasons %}
                    <div class="episode-list" style="margin-bottom: 20px;">
                        ...
                        {% if season_pack and season_pack.download_link %}
                            <button class="action-btn watch-btn" data-url="{{ season_pack.download_link }}">
                                <span>Watch All Episodes</span><i class="fas fa-play"></i>
                            </button>
                        {% endif %}
                        ...
                        {% for ep in episodes_for_season | sort(attribute='episode_number') %}
                            {% if ep.watch_link %}
                                <button class="action-btn watch-btn" data-url="{{ ep.watch_link }}">
                                    <span>Episode {{ ep.episode_number }}: {{ ep.title or 'Watch Now' }}</span><i class="fas fa-play"></i>
                                </button>
                            {% endif %}
                        {% endfor %}
                    </div>
            ...
            <!-- ম্যানুয়াল লিংকের জন্য বাটন -->
            {% if movie.manual_links %}
                <div class="link-group" style="margin-top: 20px;">
                    <h3>More Links</h3>
                    {% for link in movie.manual_links %}
                         <a href="{{ url_for('wait_page', target=quote(link.url)) }}" class="action-btn"><span>{{ link.name }}</span><i class="fas fa-external-link-alt"></i></a>
                    {% endfor %}
                </div>
            {% endif %}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // --- [নতুন] ভিডিও প্লেয়ার মডাল JavaScript ---
        ...
        function closeModal() {
            ...
            videoPlayer.src = ''; // ভিডিও বন্ধ করে দেয় এবং রিসোর্স লোড বন্ধ করে
        }

        closeModalBtn.addEventListener('click', closeModal);
        videoModal.addEventListener('click', function(event) {
            // যদি মডালের বাইরের কালো অংশে ক্লিক করা হয়, তাহলে মডাল বন্ধ হবে
...
# --- [পরিবর্তিত] Admin Panel Template for single G-Drive links ---
admin_html = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="admin-container">
    ...
    <form method="post">
        ...
        <fieldset><legend>Core Details</legend>
            ...
            <div class="form-group"><label>Screenshots (Paste one URL per line):</label><textarea name="screenshots" rows="5"></textarea></div>
            ...
            <div class="form-group"><label>OTT Platform:</label><select name="ott_platform"><option value="None">None</option>{% for platform in ott_list %}<option value="{{ platform.name }}">{{ platform.name }}</option>{% endfor %}</select></div>
        ...
        <div id="movie_fields">
            <fieldset><legend>Movie Links (Google Drive)</legend>
                ...
                <div class="form-group"><label>BLU-RAY Link:</label><input type="url" name="link_BLU-RAY"></div>
            </fieldset>
        </div>
        <div id="episode_fields" style="display: none;">
            <fieldset><legend>Series Links (Google Drive)</legend>
                <label>Complete Season Packs:</label><div id="season_packs_container"></div><button type="button" onclick="addSeasonPackField()" class="btn btn-secondary"><i class="fas fa-plus"></i> Add Season Pack</button><hr style="margin: 20px 0;">
                <label>Individual Episodes:</label><div id="episodes_container"></div><button type="button" onclick="addEpisodeField()" class="btn btn-secondary"><i class="fas fa-plus"></i> Add Episode</button>
            </fieldset>
        </div>

        <fieldset><legend>Manual External Buttons (for other links)</legend><div id="manual_links_container"></div><button type="button" onclick="addManualLinkField()" class="btn btn-secondary"><i class="fas fa-plus"></i> Add Manual Button</button></fieldset>
    ...
    <div class="table-container">
        <table>
            ...
            <tr>
                <td>{{ req.name }}</td><td style="white-space: pre-wrap; min-width: 200px;">{{ req.info }}</td>
    ...
    <div class="management-section">
        <div style="flex: 1; min-width: 300px;">
            ...
            <form method="post"><input type="hidden" name="form_action" value="add_category"><fieldset><legend>Add New Category</legend><div class="form-group"><label>Category Name:</label><input type="text" name="category_name" required></div><button type="submit" class="btn btn-primary"><i class="fas fa-plus"></i> Add Category</button></fieldset></form>
            <div class="management-list"><h3>Existing Categories</h3>{% for cat in categories_list %}<div class="management-item"><span>{{ cat.name }}</span><a href="{{ url_for('delete_category', cat_id=cat._id) }}" onclick="return confirm('Are you sure?')" class="btn btn-danger" style="padding: 5px 10px; font-size: 0.8rem;">Delete</a></div>{% endfor %}</div>
        </div>
        <div style="flex: 1; min-width: 300px;">
            <h2><i class="fas fa-tv"></i> OTT Platform Management</h2>
            <form method="post"><input type="hidden" name="form_action" value="add_platform"><fieldset><legend>Add New Platform</legend><div class="form-group"><label>Platform Name:</label><input type="text" name="platform_name" required></div><div class="form-group"><label>Logo URL:</label><input type="url" name="platform_logo_url" required></div><button type="submit" class="btn btn-primary"><i class="fas fa-plus"></i> Add Platform</button></fieldset></form>
            <div class="management-list"><h3>Existing Platforms</h3>{% for platform in ott_list %}<div class="management-item"><span>{{ platform.name }}</span><a href="{{ url_for('delete_platform', platform_id=platform._id) }}" onclick="return confirm('Are you sure?')" class="btn btn-danger" style="padding: 5px 10px; font-size: 0.8rem;">Delete</a></div>{% endfor %}</div>
        </div>
    </div>
    <hr>

    <h2><i class="fas fa-bullhorn"></i> Advertisement Management</h2>
    <form method="post"><input type="hidden" name="form_action" value="update_ads"><fieldset><legend>Global Ad Codes</legend><div class="form-group"><label>Header Script:</label><textarea name="ad_header" rows="4">{{ ad_settings.ad_header or '' }}</textarea></div><div class="form-group"><label>Body Top Script:</label><textarea name="ad_body_top" rows="4">{{ ad_settings.ad_body_top or '' }}</textarea></div><div class="form-group"><label>Footer Script:</label><textarea name="ad_footer" rows="4">{{ ad_settings.ad_footer or '' }}</textarea></div></fieldset><fieldset><legend>In-Page Ad Units</legend><div class="form-group"><label>Homepage Ad:</label><textarea name="ad_list_page" rows="4">{{ ad_settings.ad_list_page or '' }}</textarea></div><div class="form-group"><label>Details Page Ad:</label><textarea name="ad_detail_page" rows="4">{{ ad_settings.ad_detail_page or '' }}</textarea></div><div class="form-group"><label>Wait Page Ad:</label><textarea name="ad_wait_page" rows="4">{{ ad_settings.ad_wait_page or '' }}</textarea></div></fieldset><button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Save Ad Settings</button></form>
    <hr>
</div>
...
<script>
    ...
    let debounceTimer; const searchInput = document.getElementById('admin-live-search'); const tableBody = document.getElementById('content-table-body'); searchInput.addEventListener('input', () => { clearTimeout(debounceTimer); debounceTimer = setTimeout(() => { const query = searchInput.value.trim(); tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center;">Loading...</td></tr>'; fetch(`/admin/api/live_search?q=${encodeURIComponent(query)}`).then(response => response.json()).then(data => { tableBody.innerHTML = ''; if (data.length > 0) { data.forEach(movie => { const row = `<tr><td><input type="checkbox" name="selected_ids" value="${movie._id}" class="row-checkbox"></td><td>${movie.title}</td><td>${movie.type.charAt(0).toUpperCase() + movie.type.slice(1)}</td><td class="action-buttons"><a href="/edit_movie/${movie._id}" class="btn btn-edit">Edit</a><a href="/delete_movie/${movie._id}" onclick="return confirm('Are you sure?')" class="btn btn-danger">Delete</a></td></tr>`; tableBody.innerHTML += row; }); } else { tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center;">No content found.</td></tr>'; } }).catch(error => { console.error('Error:', error); tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center;">Error loading results.</td></tr>'; }); }, 400); });
...
# --- [পরিবর্তিত] Edit Page Template for single G-Drive links ---
edit_html = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="admin-container">
  ...
  <div class="page-header">
    ...
    {% if movie.tmdb_id %}<button type="button" class="btn btn-sync" id="resync-btn" onclick="resyncTmdb('{{ movie.tmdb_id }}', '{{ movie.type }}')"><i class="fas fa-sync-alt"></i> Re-sync with TMDB</button>{% endif %}
  </div>
  <form method="post">
    <fieldset><legend>Core Details</legend>
        ...
        <div class="form-group"><label>Screenshots (one URL per line):</label><textarea name="screenshots" rows="5">{{ movie.screenshots|join('\n') if movie.screenshots }}</textarea></div>
        ...
        <div class="form-group"><label>OTT Platform:</label><select name="ott_platform"><option value="None">None</option>{% for platform in ott_list %}<option value="{{ platform.name }}" {% if movie.ott_platform == platform.name %}selected{% endif %}>{{ platform.name }}</option>{% endfor %}</select></div>
    ...
    <div id="movie_fields">
        <fieldset><legend>Movie Links (Google Drive)</legend>
            ...
            <div class="form-group"><label>480p Link:</label><input type="url" name="link_480p" value="{{ links_480p.url if links_480p else '' }}"></div>
            <div class="form-group"><label>720p Link:</label><input type="url" name="link_720p" value="{{ links_720p.url if links_720p else '' }}"></div>
            <div class="form-group"><label>1080p Link:</label><input type="url" name="link_1080p" value="{{ links_1080p.url if links_1080p else '' }}"></div>
            <div class="form-group"><label>BLU-RAY Link:</label><input type="url" name="link_BLU-RAY" value="{{ links_bluray.url if links_bluray else '' }}"></div>
        </fieldset>
    </div>
    <div id="episode_fields" style="display: none;">
      <fieldset><legend>Series Links (Google Drive)</legend>
        <label>Complete Season Packs:</label><div id="season_packs_container">
        {% if movie.type == 'series' and movie.season_packs %}{% for pack in movie.season_packs|sort(attribute='season_number') %}<div class="dynamic-item"><button type="button" onclick="this.parentElement.remove()" class="btn btn-danger">X</button><div class="form-group"><label>Season No.</label><input type="number" name="season_pack_number[]" value="{{ pack.season_number }}" required></div><div class="form-group"><label>Complete Season G-Drive Link</label><input type="url" name="season_pack_link[]" value="{{ pack.download_link or '' }}"></div></div>{% endfor %}{% endif %}
    ...
    <fieldset><legend>Manual External Buttons</legend><div id="manual_links_container">
    ...
    <div class="update-options">
        <div class="checkbox-group"><label><input type="checkbox" name="send_notification" id="send_notification_checkbox" checked> Send Update Notification?</label></div>
        <div class="form-group" id="notification_text_container" style="display: none;"><label for="custom_notification_text">Custom Notification Text (Optional):</label><input type="text" name="custom_notification_text" id="custom_notification_text" placeholder="Auto-generated if left empty. e.g., S01 [EP 01-10 ADDED]"></div>
        <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Update Content</button>
    </div>
  </form>
</div>
<script>
    function toggleFields() { var isSeries = document.getElementById('content_type').value === 'series'; document.getElementById('episode_fields').style.display = isSeries ? 'block' : 'none'; document.getElementById('movie_fields').style.display = isSeries ? 'none' : 'block'; const notificationContainer = document.getElementById('notification_text_container'); const sendNotificationCheckbox = document.getElementById('send_notification_checkbox'); if (isSeries && sendNotificationCheckbox.checked) { notificationContainer.style.display = 'block'; } else { notificationContainer.style.display = 'none'; } }
    function addEpisodeField() { const c = document.getElementById('episodes_container'); const d = document.createElement('div'); d.className = 'dynamic-item'; d.innerHTML = `<button type="button" onclick="this.parentElement.remove()" class="btn btn-danger">X</button><div class="form-group"><label>Season:</label><input type="number" name="episode_season[]" value="1" required></div><div class="form-group"><label>Episode:</label><input type="number" name="episode_number[]" required></div><div class="form-group"><label>Title:</label><input type="text" name="episode_title[]"></div><div class="form-group"><label>G-Drive Link:</label><input type="url" name="episode_watch_link[]" required></div>`; c.appendChild(d); }
    function addSeasonPackField() { const c = document.getElementById('season_packs_container'); const d = document.createElement('div'); d.className = 'dynamic-item'; d.innerHTML = `<button type="button" onclick="this.parentElement.remove()" class="btn btn-danger">X</button><div class="form-group"><label>Season No.</label><input type="number" name="season_pack_number[]" value="1" required></div><div class="form-group"><label>Complete Season G-Drive Link</label><input type="url" name="season_pack_link[]"></div>`; c.appendChild(d); }
    ...
    async function resyncTmdb(tmdbId, mediaType) { const btn = document.getElementById('resync-btn'); btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Syncing...'; try { const response = await fetch(`/admin/api/resync_tmdb?id=${tmdbId}&type=${mediaType}`); if (!response.ok) throw new Error('Failed to fetch data'); const data = await response.json(); document.getElementById('title').value = data.title || ''; document.getElementById('overview').value = data.overview || ''; document.getElementById('poster').value = data.poster || ''; document.getElementById('backdrop').value = data.backdrop || ''; document.getElementById('genres').value = data.genres ? data.genres.join(', ') : ''; alert('Data synced successfully! Please review and save.'); } catch (e) { console.error(e); alert('Error syncing data.'); } finally { btn.disabled = false; btn.innerHTML = '<i class="fas fa-sync-alt"></i> Re-sync with TMDB'; } }
    function formatSeriesInfoJS(episodes, seasonPacks) { let infoParts = []; if (seasonPacks && seasonPacks.length > 0) { const sortedPacks = seasonPacks.sort((a, b) => a.season_number - b.season_number); sortedPacks.forEach(pack => { const seasonNum = String(pack.season_number).padStart(2, '0'); infoParts.push(`S${seasonNum} [COMPLETE SEASON]`); }); } if (episodes && episodes.length > 0) { const episodesBySeason = {}; episodes.forEach(ep => { if (!episodesBySeason[ep.season]) episodesBySeason[ep.season] = []; episodesBySeason[ep.season].push(ep.episode_number); }); Object.keys(episodesBySeason).sort((a, b) => a - b).forEach(season => { const epNums = episodesBySeason[season].sort((a, b) => a - b); if (epNums.length === 0) return; const seasonNum = String(season).padStart(2, '0'); const firstEp = String(epNums[0]).padStart(2, '0'); const lastEp = String(epNums[epNums.length - 1]).padStart(2, '0'); const epRange = (firstEp === lastEp) ? `EP${firstEp}` : `EP${firstEp}-${lastEp}`; infoParts.push(`S${seasonNum} [${epRange} ADDED]`); }); } return infoParts.join(' & '); }
    function autoGenerateNotificationText() { if (document.getElementById('content_type').value !== 'series') { document.getElementById('custom_notification_text').value = ''; return; } const oldEpisodes = JSON.parse('{{ movie.episodes|tojson|safe }}' || '[]'); const oldSeasonPacks = JSON.parse('{{ movie.season_packs|tojson|safe }}' || '[]'); const oldEpIds = new Set(oldEpisodes.map(ep => `${ep.season}-${ep.episode_number}`)); const oldPackIds = new Set(oldSeasonPacks.map(p => p.season_number)); const currentEps = []; document.querySelectorAll('#episodes_container .dynamic-item').forEach(item => { const season = item.querySelector('[name="episode_season[]"]').value; const episode = item.querySelector('[name="episode_number[]"]').value; if (season && episode) currentEps.push({ season: parseInt(season), episode_number: parseInt(episode) }); }); const currentPacks = []; document.querySelectorAll('#season_packs_container .dynamic-item').forEach(item => { const season = item.querySelector('[name="season_pack_number[]"]').value; if (season) currentPacks.push({ season_number: parseInt(season) }); }); const newlyAddedEps = currentEps.filter(ep => !oldEpIds.has(`${ep.season}-${ep.episode_number}`)); const newlyAddedPacks = currentPacks.filter(p => !oldPackIds.has(p.season_number)); const generatedText = formatSeriesInfoJS(newlyAddedEps, newlyAddedPacks); document.getElementById('custom_notification_text').value = generatedText; }
    function setupMutationObserver() { const episodesContainer = document.getElementById('episodes_container'); const packsContainer = document.getElementById('season_packs_container'); const observerCallback = (mutationsList, observer) => { for(const mutation of mutationsList) { if (mutation.type === 'childList') autoGenerateNotificationText(); } }; const observer = new MutationObserver(observerCallback); const config = { childList: true }; if (episodesContainer) observer.observe(episodesContainer, config); if (packsContainer) observer.observe(packsContainer, config); document.body.addEventListener('input', (event) => { if (event.target.matches('[name="episode_season[]"], [name="episode_number[]"], [name="season_pack_number[]"]')) { autoGenerateNotificationText(); } }); }
    document.addEventListener('DOMContentLoaded', function() { toggleFields(); setupMutationObserver(); const checkbox = document.getElementById('send_notification_checkbox'); checkbox.addEventListener('change', toggleFields); });
</script>
</body></html>
"""
...
# --- [পরিবর্তিত] Admin Route with G-Drive link conversion ---
    ...
    if request.method == "POST":
        ...
        if form_action == "update_ads":
            ...
            movie_data = {
                ...
                "overview": request.form.get("overview").strip(), "screenshots": screenshots_list, "language": request.form.get("language").strip() or None,
                "genres": [g.strip() for g in request.form.get("genres", "").split(',') if g.strip()], "categories": request.form.getlist("categories"),
                "episodes": [], "links": [], "season_packs": [], "manual_links": [], "created_at": datetime.utcnow(), "updated_at": datetime.utcnow(),
                "view_count": 0, "tmdb_id": tmdb_id if tmdb_id else None, "is_completed": is_completed
            }
            ...
            if content_type == "movie":
                ...
                for q in qualities:
                    ...
                    if link: movie_data["links"].append({"quality": q, "url": convert_gdrive_link_to_embed(link)})
            else:
                sp_nums, sp_links = request.form.getlist('season_pack_number[]'), request.form.getlist('season_pack_link[]')
                movie_data['season_packs'] = [{"season_number": int(sp_nums[i]), "download_link": convert_gdrive_link_to_embed(sp_links[i])} for i in range(len(sp_nums)) if sp_nums[i] and sp_links[i]]
                ...
                movie_data['episodes'] = [{"season": int(s[i]), "episode_number": int(n[i]), "title": t[i].strip(), "watch_link": convert_gdrive_link_to_embed(l[i])} for i in range(len(s)) if s[i] and n[i] and l[i]]
            ...
            if result.inserted_id:
                series_info = None
                if movie_data['type'] == 'series': series_info = format_series_info(movie_data.get('episodes', []), movie_data.get('season_packs', []))
...
# --- [পরিবর্তিত] Edit Route with G-Drive link conversion ---
...
def edit_movie(movie_id):
    ...
    if request.method == "POST":
        ...
        update_data = {
            ...
            "overview": request.form.get("overview").strip(), "screenshots": screenshots_list, "language": request.form.get("language").strip() or None,
            "genres": [g.strip() for g in request.form.get("genres").split(',') if g.strip()], "categories": request.form.getlist("categories"),
            "updated_at": datetime.utcnow(), "is_completed": is_completed
        }
        ...
        series_update_info_str, custom_notification_text = None, request.form.get("custom_notification_text", "").strip()

        if content_type == "series":
            sp_nums, sp_links = request.form.getlist('season_pack_number[]'), request.form.getlist('season_pack_link[]')
            update_data['season_packs'] = [{"season_number": int(sp_nums[i]), "download_link": convert_gdrive_link_to_embed(sp_links[i])} for i in range(len(sp_nums)) if sp_nums[i]]
            s, n, t, l = request.form.getlist('episode_season[]'), request.form.getlist('episode_number[]'), request.form.getlist('episode_title[]'), request.form.getlist('episode_watch_link[]')
            update_data["episodes"] = [{"season": int(s[i]), "episode_number": int(n[i]), "title": t[i].strip(), "watch_link": convert_gdrive_link_to_embed(l[i])} for i in range(len(s)) if s[i] and n[i] and l[i]]
            update_query.setdefault("$unset", {})["links"] = ""

            if custom_notification_text: series_update_info_str = custom_notification_text
            else:
                ...
                if newly_added_eps or newly_added_packs: series_update_info_str = format_series_info(newly_added_eps, newly_added_packs)
        else: # Movie
            qualities = ["480p", "720p", "1080p", "BLU-RAY"]
            update_data["links"] = []
            for q in qualities:
                link = request.form.get(f"link_{q}")
                if link: update_data["links"].append({"quality": q, "url": convert_gdrive_link_to_embed(link)})
            update_query.setdefault("$unset", {})["episodes"] = ""
            update_query.setdefault("$unset", {})["season_packs"] = ""
        
        if ott_platform and ott_platform != "None": update_query["$set"]["ott_platform"] = ott_platform
        else: update_query.setdefault("$unset", {})["ott_platform"] = ""
        ...
        if request.form.get('send_notification'):
            ...
            send_telegram_notification(notification_data, obj_id, notification_type='update', series_update_info=series_update_info_str)
...
def api_search_tmdb():
    ...
    try:
        ...
        if search_year:
            movie_params = {**base_params, 'primary_release_year': search_year}
            ...
            tv_params = {**base_params, 'first_air_date_year': search_year}
...
def api_resync_tmdb():
    tmdb_id, media_type = request.args.get('id'), request.args.get('type') 
    if not tmdb_id or not media_type: return jsonify({"error": "TMDB ID and media type are required"}), 400
